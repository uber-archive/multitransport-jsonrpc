        ...
        if(data.method) {
            ...
            if(this.scope[data.method] && !(this.scope[data.method].length === arglen || this.scope[data.method].blocking)) {
                    ...
                    if(error) {
                        ...
                        if(error instanceof Error) {
                            ...
                            for (var i = 0; i < keys.length; ++i) {
                                var key = keys[i];
                                outObj.error.data[key] = error[key];
                            }
                            outObj.error.data.message = error.message;
                        } else {
                            outObj.error.data = error;
                        }
                    } else {
                ...
                try {
                    ...
                    var outErr = errorObject.internalError;
                    outErr.data.message = e.message ? e.message : "";
                    outErr.data.stack = e.stack ? e.stack : "";
                    var outObj = setDefaultProperty(data);
                    outObj.error = outErr;
                    callback(outObj);
                    throw e;
                ...
                errObj1.error = errorObject.methodNotFound;
                callback(errObj1);
            }
        } else {
            var errObj2 = setDefaultProperty(data);
            errObj2.error = errorObject.invalidRequest;
            callback(errObj2);
        }
    } else {
        var errObj3 = setDefaultProperty(data);
        errObj3.error = errorObject.parseError;
        callback(errObj3);
    }
};
