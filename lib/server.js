        ...
        if(data.method) {
            ...
            if(this.scope[data.method] && !(this.scope[data.method].length === arglen || this.scope[data.method].blocking)) {
                ...
                try {
                    ...
                    var outObj = { error: outErr };
                    if(data.id) outObj.id = data.id;
                    callback(outObj);
                    throw e;
                }
            } else {
                callback({error:{code: errorCode.methodNotFound, message:"Requested method does not exist."}, id: data.hasOwnProperty('id') ? data.id : -1});
            }
        } else {
            callback({error:{code: errorCode.invalidRequest, message:"Did not receive valid JSON-RPC data."}, id: data.hasOwnProperty('id') ? data.id : -1});
        }
    } else {
        callback({error:{code: errorCode.parseError, message:"Did not receive valid JSON-RPC data."}, id: data.hasOwnProperty('id') ? data.id : -1});
    }
};
