    ...
    if(data instanceof Object) {
        if(data.method) {
            ...
            if(this.scope[data.method] && !(this.scope[data.method].length === arglen || this.scope[data.method].blocking)) {
                ...
                callback({result:null, error:{message:"Requested method does not exist."}, id: data.hasOwnProperty('id') ? data.id : -1});
            }
        } else {
            callback({result:null, error:{message:"Did not receive valid JSON-RPC data."}, id: data.hasOwnProperty('id') ? data.id : -1});
        }
    } else {
        callback({result:null, error:{message:"Did not receive valid JSON-RPC data."}, id: data.hasOwnProperty('id') ? data.id : -1});
    }
