...
JSONRPC.prototype.handleJSON = function(request, response) {
    var DATA = "";
    ...
    request.addListener('end', function() {
            ...
            if(this.scope[data.method] && !this.scope[data.method].blocking) {
            ...
            try {
                if(data.params && data.params instanceof Array) {
                    ...
                    this.scope[data.method].apply(this.scope, [data.params, callback]);
                } else {
                    this.scope[data.method].apply(this.scope, [callback]);
                }
            } catch(e) {
                var outErr = {};
                if(e.message) {
            ...
            // above nonblocking code.
                ...
                try {
                    if(data.params && data.params instanceof Array) {
                        result = this.scope[data.method].apply(this.scope, data.params);
                    } else if(data.params) {
                        result = this.scope[data.method].apply(this.scope, [data.params]);
                    } else {
                        result = this.scope[data.method].apply(this.scope, []);
                    }
                    if(data.id) {
...
JSONRPC.prototype.returnVal = function(response, retVal) {
